...
const chipColor = (i: CapaItem) =>
  ...
  : i.derived === 'Cancelled' ? 'var(--baseline)'
  : i.overdue ? 'var(--critical)'
  : i.derived === 'Waiting Verification' ? 'var(--warning)'
  : 'var(--accent)'

export function CalendarView({ items, onOpen }: { items: CapaItem[] | null; onOpen: (id: string) => void }) {
  ...
  const byDay = useMemo(() => {
    ...
    ;(items ?? []).forEach((i) => map.set(i.dueDate, [...(map.get(i.dueDate) ?? []), i]))
    return map
  }, [items])
  ...
  if (items === null) return <Card className="p-5"><Skeleton className="h-96 w-full" /></Card>
      ...
      <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-1.5">
          <span className="mr-2 hidden items-center gap-3 text-2xs text-muted md:flex">
            {[['Upcoming', 'var(--accent)'], ['Overdue', 'var(--critical)'], ['Verification', 'var(--warning)'], ['Done', 'var(--good)']].map(([l, c]) => (
              <span key={l} className="inline-flex items-center gap-1"><span className="h-2 w-2 rounded-full" style={{ background: c }} />{l}</span>
            ))}
          </span>
          <Button variant="secondary" size="sm" icon={<ChevronLeft size={13} />} onClick={() => setCursor(new Date(cursor.getFullYear(), cursor.getMonth() - 1, 1))} aria-label="Previous month" />
          <Button variant="secondary" size="sm" onClick={() => { const d = new Date(); setCursor(new Date(d.getFullYear(), d.getMonth(), 1)) }}>Today</Button>
          <Button variant="secondary" size="sm" icon={<ChevronRight size={13} />} onClick={() => setCursor(new Date(cursor.getFullYear(), cursor.getMonth() + 1, 1))} aria-label="Next month" />
        </div>
      </div>
              ...
              <div className="mt-1 space-y-1">
                {dayItems.slice(0, 3).map((i) => (
                  <button
                    key={i.id}
                    onClick={() => onOpen(i.id)}
                    title={`${i.code} · ${i.title} — ${i.owner}`}
                    className="flex w-full items-center gap-1 rounded-md border px-1.5 py-0.5 text-left text-2xs font-medium text-ink transition-colors hover:bg-accent-soft"
                    style={{ borderColor: chipColor(i) }}
                  >
                    <span className="h-1.5 w-1.5 shrink-0 rounded-full" style={{ background: chipColor(i) }} />
                    <span className="truncate">{i.code}</span>
                  </button>
                ))}
